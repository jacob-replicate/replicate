class ColdEmailScheduler
  ...
  def fetch_contacts
    contacts = Contact.us.enriched.where(email_queued_at: nil, contacted_at: nil, unsubscribed: false).where("score >= ?", @min_score).order(score: :desc).limit(1000).to_a
