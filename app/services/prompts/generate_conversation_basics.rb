module Prompts
  class GenerateConversationBasics < Prompts::Base
    def validate(raw)
      raw_json = Prompts::Base.extract_json(raw)
      failures = []

      failures << "raw_json_not_hash" unless raw_json.is_a?(Hash)
      failures << "missing_name" if raw_json["conversation_name"].to_s.strip.empty?
      failures << "missing_description" if raw_json["conversation_description"].to_s.strip.empty?
      failures << "missing_code" if raw_json["conversation_code"].to_s.strip.empty?
      failures << "missing_refined_intent" if raw_json["refined_generation_intent"].to_s.strip.empty?

      code = raw_json["conversation_code"].to_s.strip
      unless code.match?(/\A[a-z0-9]+(-[a-z0-9]+)*\z/)
        failures << "invalid_code_format"
      end
      failures << "code_too_long" if code.length > 25

      failures << "name_too_long" if raw_json["conversation_name"].to_s.length > 100

      desc_length = raw_json["conversation_description"].to_s.length
      failures << "description_too_long" if desc_length > 200
      ...
      text_fields = ["conversation_name", "conversation_description"]
