module MessageGenerators
  class Incident < MessageGenerators::Base
    def deliver_intro
      ...
      reply = if @conversation.referring_conversation.present?
        ...
        CachedPrompt.call(Prompts::IncidentIntro, generation_intent: @conversation.generation_intent)
      end
      ...
      title = CachedPrompt.call(Prompts::IncidentTitle, generation_intent: @conversation.generation_intent)
    ...
    def deliver_reply
      ...
      if latest_message == "Give me a hint"
        ...
        prompt = Prompts::IncidentExplain
        multiple_choice_options = 3
      ...
      reply = prompt.new(context: { custom_instructions: custom_instructions, cta: question_format }, message_history: @conversation.message_history).call
      broadcast_to_web(type: "element", message: reply, user_generated: false)
    ...
    def deliver_multiple_choice_options(count, reply, cacheable = false)
      options = if cacheable
        CachedPrompt.call(Prompts::MultipleChoiceOptions, generation_intent: @conversation.generation_intent, context: { max: count, most_recent_message: reply })
      else
        Prompts::MultipleChoiceOptions.new(context: { max: count, most_recent_message: reply }, message_history: @conversation.message_history).call
      end
