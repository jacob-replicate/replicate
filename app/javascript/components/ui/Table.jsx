...
export const TableRow = ({ children, href, onClick, target, isFirst = false, className = '', ...rest }) => {
  ...
  if (href) {
    return (
      <div className={borderClass} {...rest}>
        <a href={href} onClick={onClick} target={target} className={`flex items-center px-4 py-2.5 ${hoverClass} ${className}`}>
          {children}
        </a>
      </div>
    )
  ...
  return (
    <div className={`${borderClass} ${hoverClass} transition-colors ${className}`} {...rest}>
