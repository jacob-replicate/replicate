...
const CategorySection = ({ name, topics, variant = 'default', expandedTopicCode, onTopicClick, onBackToCategory, onRefetch, isAdmin }) => {
  const headerClass = variant === 'uncategorized'
    ? 'text-[13px] font-medium text-red-400 tracking-wide'
    : 'text-[15px] font-medium text-white tracking-wide'
      ...
      {expandedTopic ? (
        ...
        <Card className={cardClass}>
          <CardBody>
            ...
            <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-zinc-700/50">
              <h2 className={headerClass}>{name}</h2>
              {progressPct > 0 && (
                <div className="h-1.5 bg-zinc-700 rounded-full overflow-hidden w-[100px]">
                  <div
